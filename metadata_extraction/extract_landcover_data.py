"""
Extract land cover data from MODIS Terra-Aqua raster files for fungi observations.

This script processes a CSV file containing fungi observation metadata and extracts
land cover classification data for each observation from MODIS Terra-Aqua raster files.
The process:

1. Loads observation metadata from a CSV file
2. Divides the geographic extent into tiles for parallel processing
3. For each tile:
   - Extracts land cover class values for observations within the tile
...
Required files:
    ...
    - MODIS Terra-Aqua land cover raster file (.tif)

Example usage:
    python extract_landcover_data.py
"""
...
if __name__ == "__main__":
    ...
    raster = "landcover"  # Name for the extracted data column
    ...
    # Extract land cover data in parallel
    print(f"Extracting land cover data using {n_jobs} processes...")
    ...
    # Add land cover data to output metadata
