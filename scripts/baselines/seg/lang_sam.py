...
class LangSAM():
    ...
    def build_sam(self):
        ckpt_repo_id = "ybelkada/segment-anything"
        ckpt_filename = "checkpoints/sam_vit_b_01ec64.pth"
    ...
    def predict_sam(self, image_pil, boxes):
        ...
        masks, iou_preds, _ = self.sam.predict_torch(
            point_coords=None,
            point_labels=None,
            boxes=transformed_boxes.to(self.sam.device),
            multimask_output=False,
        )
        return masks.cpu(), iou_preds

    def predict(self, image_pil, text_prompt, box_threshold=0.3, text_threshold=0.25):
        boxes, gdino_logits, phrases = self.predict_dino(image_pil, text_prompt, box_threshold, text_threshold)
        masks = torch.tensor([])
        seg_confs = []
        if len(boxes) > 0:
            masks, ious = self.predict_sam(image_pil, boxes)
            masks = masks.squeeze(1)
            seg_confs = ious.cpu().numpy()
        return masks, boxes, phrases, gdino_logits, seg_confs
